...
function setupEventListeners() {
    ...
    document.querySelectorAll('.nav-menu a').forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            document.querySelectorAll('.nav-menu a').forEach(a => a.classList.remove('aktiv'));
...
function handleButtonClick(e) {
    // Admin bejelentkezés
    if (e.target.id === 'adminBejelentkezesGomb' || 
        (e.target.closest('button') && e.target.closest('button').id === 'adminBejelentkezesGomb')) {
        e.preventDefault();
        adminBejelentkezes();
    }
    
    // Admin kijelentkezés
    if (e.target.id === 'adminKijelentkezesGomb' || 
        (e.target.closest('button') && e.target.closest('button').id === 'adminKijelentkezesGomb')) {
        e.preventDefault();
        adminKijelentkezes();
    }
    
    // Ötlet törlése
    if (e.target.classList.contains('otlet-torles') || 
        (e.target.closest('button') && e.target.closest('button').classList.contains('otlet-torles'))) {
        const otletId = e.target.dataset.otletId || e.target.closest('button').dataset.otletId;
...
// Ötlet beküldés oldal - KIJAVÍTVA
function generateOtletBekuldes() {
    return `
        <div class="kartya">
            <h2 class="kartya-cim"><i class="fas fa-plus-circle"></i> Új ötlet beküldése</h2>
...
function generateOtletek() {
    if (appState.otletek.length === 0) {
        return `
            <h2 class="kartya-cim"><i class="fas fa-list"></i> Összes ötlet (${appState.otletek.length})</h2>
            ...
            <div class="otlet-kartyak" id="otletLista">
                ${appState.otletek.map(otlet => generateOtletKartya(otlet)).join('')}
            </div>
        </div>
        ...
        <div class="otlet-kartya ${otlet.kategoria}">
            <div class="otlet-fejlec">
                <div>
                    ...
                    <span class="otlet-kategoria">${kategoriaSzoveg[otlet.kategoria]}</span>
                </div>
            ...
            <div class="otlet-lablec">
                ...
                ${otlet.diak_osztalya ? `<span><i class="fas fa-graduation-cap"></i> ${otlet.diak_osztalya}</span>` : ''}
            </div>
        </div>
    `;
}
    ...
    return `
        <div class="kartya">
            <h2 class="kartya-cim">
                ...
                <button id="adminKijelentkezesGomb" class="gomb gomb-warning" style="float: right; padding: 8px 15px;">
            ...
            <div class="admin-lista">
                ...
                ${appState.otletek.map(otlet => `
                    <div class="admin-sor" data-otlet-id="${otlet.id}">
                        <div><strong>${otlet.otlet_cime}</strong></div>
                        <div>${otlet.diak_neve}</div>
                        <div>${kategoriaSzoveg[otlet.kategoria] || otlet.kategoria}</div>
                        <div>
                            <select class="statusz-valtoztatas" data-otlet-id="${otlet.id}">
                                <option value="beküldve" ${otlet.statusz === 'beküldve' ? 'selected' : ''}>Beküldve</option>
                                <option value="átnézés alatt" ${otlet.statusz === 'átnézés alatt' ? 'selected' : ''}>Átnézés alatt</option>
                                <option value="elfogadva" ${otlet.statusz === 'elfogadva' ? 'selected' : ''}>Elfogadva</option>
                                <option value="elutasítva" ${otlet.statusz === 'elutasítva' ? 'selected' : ''}>Elutasítva</option>
                            </select>
                        </div>
                        <div>
                            <button class="gomb gomb-danger otlet-torles" data-otlet-id="${otlet.id}" style="padding: 5px 10px; font-size: 0.9rem;">
...
async function submitOtlet() {
    ...
    console.log('Adatok:', { diakNeve, diakOsztalya, otletCime, otletKategoria, otletLeirasa });
    ...
    try {
        ...
        if (data.success) {
            ...
            // Újra betöltjük az ötleteket
            await loadOtletek();
            
            // 3 másodperc múlva átnavigálunk a főoldalra
            setTimeout(() => {
        ...
        showOtletUzenet('Hiba történt az ötlet beküldése során. Próbáld újra!', 'error');
    }
}
...
function showOtletUzenet(message, type = 'info') {
    ...
    if (messageDiv) {
        const className = type === 'success' ? 'uzi-siker' : type === 'error' ? 'uzi-hiba' : 'uzi-info';
...
function generateFooldal() {
    return `
  <div class="kartya">
            <h2 class="kartya-cim"><i class="fas fa-home"></i> Üdvözöljük az Digitális Ötletládában!</h2>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; margin-top: 30px;">
                <div class="stat-kartya" style="text-align: center; padding: 20px; background-color: #c5dffd; border-radius: var(--border-radius);">
                    <h3 style="font-size: 2rem; color: blue;">${appState.otletek.length}</h3>
                    <p>Összes ötlet száma</p>
                </div>
                <div style="text-align: center; padding: 20px; background-color: #cdf8d8; border-radius: var(--border-radius);">
                    <h3 style="font-size: 2rem; color: green;">${appState.otletek.filter(o => o.statusz === 'elfogadva').length}</h3>
                    <p>Elfogadott ötlet</p>
                </div>
                <div style="text-align: center; padding: 20px; background-color: #d6d1c9; border-radius: var(--border-radius);">
                    <h3 style="font-size: 2rem; color: dark-grey;">${appState.otletek.filter(o => o.statusz === 'beküldve').length}</h3>
                    <p>Beküldött (át nem nézett) ötletek</p>
                </div>
                <div style="text-align: center; padding: 20px; background-color: #fde2ca; border-radius: var(--border-radius);">
                    <h3 style="font-size: 2rem; color: orange;">${appState.otletek.filter(o => o.statusz === 'átnézés alatt').length}</h3>
                    <p>Átnézés alatti ötletek</p>
                </div>
                <div style="text-align: center; padding: 20px; background-color: #fab3b3; border-radius: var(--border-radius);">
                    <h3 style="font-size: 2rem; color: red;">${appState.otletek.filter(o => o.statusz === 'elutasítva').length}</h3>
                    <p>Elutasított ötletek</p>
                </div>
            </div>
        </div>
        
        <div class="kartya">
            <h2 class="kartya-cim"><i class="fas fa-bolt"></i> Legfrissebb ötletek</h2>
            <div class="otlet-kartyak">
                ${appState.otletek.slice(0, 3).map(otlet => generateOtletKartya(otlet)).join('')}
            </div>
        </div>
    `;
}
